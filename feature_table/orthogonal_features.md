...
The default is `deltaz_integrated_abs`, matching `fit_monotonic_gbm.py`'s
`DELTAZ_COL`. This is a deliberate fix relative to `fit_f_physics.py`, which
orthogonalized against `deltaz_mean_all` (a *different* column) — appropriate for
its own pipeline but wrong for a GBM whose primary feature is
`deltaz_integrated_abs`. Change `--deltaz-col` explicitly if your downstream
model uses a different summary statistic.
