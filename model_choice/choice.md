...
The script lives in the `model_choice/` folder. Run from the repo root.

**Train all four models on the shared split and report per-split MAE / RMSE:**
...
This trains LightGBM, XGBoost, CatBoost, and their equal-weight ensemble on the
same split and prints the per-split MAE / RMSE table shown above (train / val /
test_early / later_life / all). The `later_life` rows are the basis for the
decision recorded in this document.

Notes:
- `fit_earlylife_ensemble.py`'s `--error-threshold` (default 3.0) affects only
  the threshold-dependent summary columns, **not** the MAE / RMSE reported here.
- Numbers in this document are from seed 0. The later-life ranking is stable
  across seeds; re-running with a different `--seed` may shift third-decimal
  values without changing the ordering.