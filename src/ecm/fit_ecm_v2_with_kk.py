...
R_1ARC_MAX_PLAUSIBLE = 150.0  # mOhm — the 1-arc R_1arc's own 99.9th percentile on the full
                              # dataset is ~106 mOhm; a fit landing above 150 is a degenerate
                              # optimizer solution, not a genuine late-life resistance
...
class FitResult:
    ...
    r_ohm_implausible: bool = False       # 2-arc R_ohm degenerate (near-zero)
    fit_1arc_implausible: bool = False    # 1-arc PRIMARY feature degenerate: R_ohm_1arc near-zero,
                                           # R_1arc absurdly large, or n_1arc physically implausible
...
def fit_one_spectrum(key: tuple, spec_df: pd.DataFrame) -> FitResult:
    ...
    result.fit_1arc_implausible = bool(
        result.R_ohm_1arc < R_OHM_MIN_PLAUSIBLE
        or result.R_1arc > R_1ARC_MAX_PLAUSIBLE
        or result.n_1arc <= CPE_BOUND_EPS or result.n_1arc >= 1 - CPE_BOUND_EPS
        or result.n_1arc < CPE_N_PLAUSIBLE_MIN
    )
